...
- A stack that `gh stack init` recorded in a linked worktree shows its badges and `gh stack`
  commands in every checkout. `gh stack` keeps its state per worktree, and the panel read only the
  main checkout's.
- The `gh stack` commands run in the worktree that recorded the stack, on the selected commit's
  stack. They used to run in the panel's own checkout, where `gh stack` answered "not part of a
  stack", or "not on any branch" from a detached HEAD.
